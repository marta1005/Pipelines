...
def preprocessor(workflow, input_table):
    ...
    ## Example of function 
    ''' 
    metadata = workflow.get_step() ## to get metadata in the current step
    ## option 1: "hardcoded"
    if metadata['method'] == "StandarScaler":
        scaler = StandardScaler() 
    ## option 2: "parametrized with catalog". Note: this option requires catalog being informed with appropiate methods
    scaler = workflow.catalog.get_method(metadata['method'])
    ''' 
